...
$(this).parent().find('UL').remove();showTree($(this).parent(),$(this).attr('rel'));$(this).parent().removeClass('collapsed').addClass('expanded');}else{$(this).parent().find('UL').slideUp({duration:o.collapseSpeed,easing:o.collapseEasing});$(this).parent().removeClass('expanded').addClass('collapsed');}}else{h($(this).attr('rel'));}
return false;});if(o.folderEvent.toLowerCase!='click')$(t).find('LI A').bind('click',function(){if($(this).parent().hasClass('directory')){newpath=$(this).attr('rel'),$('#musicpath').text(newpath),$('#MusicFolder').val(newpath),$('#MusicFolder').attr("disabled",true);temp=newpath.substring(0,(newpath.length)-1);foldername=temp.substring(temp.lastIndexOf('/')+1,temp.length);$('#RenFolder').val(foldername);}});}
